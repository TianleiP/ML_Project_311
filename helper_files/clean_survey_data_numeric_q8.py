...
def clean_data():
    ...
    # Output directories
    main_output_dir = 'data/numeric_hot_sauce_data'
    ...
    output_file = os.path.join(main_output_dir, 'numeric_hot_sauce_survey_data.csv')
    ...
    questions = {
        ...
        'Q8_hot_sauce': ['id', 'hot_sauce', 'Label']  # Now just a single numeric column
    }
    ...
    print(f"Cleaning completed. Numeric hot sauce data shape: {cleaned_data.shape}")
    print(f"Columns in numeric hot sauce data: {cleaned_data.columns.tolist()}")
    
    # Count null values
    null_counts = cleaned_data.isnull().sum()
    print("\nNull values per column (should be 0 for ingredients, price, hot_sauce, and movie):")
